using UnityEngine;
using UnityEditor;
public class SurfaceSnapObjectEditor : EditorWindow
{
    RaycastHit hit;
    float yOffset;
    int savedLayer;
    ...
    [MenuItem("Window/Snap Object")]                                                // add menu item
    static void Awake()
    {
        EditorWindow.GetWindow<SurfaceSnapObjectEditor>().Show();                   // Get existing open window or if none, make a new one
    }

    void OnGUI()
    {
        GUILayout.Label("Snap using: ", EditorStyles.boldLabel);
